...
app.post("/sharing", async (req, res) => {
  ...
  if (cooldown.get(req.user.id) && cooldown.get(req.user.id).count >= 100) return error(res, 429, "10 dakika içerisinde en fazla 100 kod paylaşabilirsin!");
  ...
  let color;
  if (code.rank === "javascript") color = "#0000ff";
  else if (code.rank === "bdfd") color = "#efff0a";
  else if (code.rank === "html") color = "#0ae7ff";
  else if (code.rank === "ready") color = "#f70aff";
  else if (code.rank === "fromyou") color = "#000000"
  const embed = new MessageEmbed()
  .setAuthor(req.user.username, member.user.avatarURL({ dynamic: true }))
  .setThumbnail(guild.iconURL({ dynamic: true }))
  .setTitle(`<a:HypeSquad:853380191891619860> • ${code.rank} kategorisinde bir kod paylaşıldı!`)
  .setDescription(`
  <a:nstagram:853410409897000990> • Kod adı: [${code.name}](https://${conf.domain}/${code.rank}/${id})
  <a:lb_cekilis:853380234371530762> • Kod Açıklaması: ${code.desc}
  <a:UwU:853380298267820032> • Kodu paylaşan: ${member.toString()}
  `)
  .setColor(color)
  channel.send(embed);
...
app.get("/javascript", async (req, res) => {
  const codeData = require("./src/schemas/code");
  const data = await codeData.find({ rank: "javascript" }).sort({ date: -1 });
  res.render("javascriptCodes", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
    moment,
...
app.get("/javascript/:codeID", async (req, res) => {
  ...
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  if (member && !member.roles.cache.has(conf.normalRole) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen bilgilendirme sayfasını okuyunuz!");
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  const codeData = require("./src/schemas/code");
  const code = await codeData.findOne({ rank: "javascript", id: codeID });
...
client.login("ODQxMzM3MTE4NzEzNTc3NTIy.YJlSQA.sHLruMcXiClG5aNLZPrmI2XEN3I")
